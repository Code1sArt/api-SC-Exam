...
\`\`\`
คะแนนเต็ม: ${input.maxScore}
ตรวจความถูกต้องของแนวคิดและอัลกอริทึม การทำงานตามโจทย์ กรณีขอบเขต และโครงสร้างลำดับการทำงานตามคะแนนเต็มของส่วนนี้
ห้ามให้หรือตัดคะแนนจากความสวยงาม การจัด format การตั้งชื่อตัวแปร ความยาวโค้ด comment หรือ coding style เว้นแต่ rubric ระบุไว้โดยตรงหรือสิ่งนั้นทำให้โปรแกรมทำงานผิด
ใช้ผลรัน test case ที่แนบในโจทย์เป็นหลักฐานประกอบ แต่ไม่ตัดคะแนนเป็นศูนย์อัตโนมัติเพียงเพราะ output ไม่ตรงแบบ exact
หากแนวคิดและผลลัพธ์หลักถูก แต่มีข้อความหรือคำเกินจาก expected output ให้คะแนนบางส่วนตามสัดส่วน โดยต้องหักคะแนนเรื่องรูปแบบ output และห้ามให้เต็ม
หากโค้ดคอมไพล์ไม่ได้หรือแนวคิดหลักผิด อาจให้คะแนนเฉพาะส่วนที่แสดงความเข้าใจได้จริงเท่านั้น
ห้ามนับคะแนน test case ซ้ำ เพราะระบบภายนอกคำนวณส่วนนั้นแยกไว้แล้ว และห้ามอ้างว่าได้รันโค้ดเอง
ให้คำแนะนำภาษาไทยที่ชัดเจนและนำไปแก้ไขได้ทันที
อธิบายผลโดยอ้างอิงผล test case แนวคิด โครงสร้าง และอัลกอริทึมเท่านั้น ห้ามกล่าวถึง AI โมเดล หรือวิธีการภายในที่ใช้ตรวจ
ตอบ JSON เท่านั้น: {"score": number, "isCorrect": boolean, "feedback": string, "confidence": number}
score ต้องอยู่ระหว่าง 0 ถึงคะแนนเต็ม และ confidence อยู่ระหว่าง 0 ถึง 1`;
    const models = await this.getModels(context.organizationId);
    return this.runGemini<GradeResult>(
